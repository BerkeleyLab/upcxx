...
namespace upcxx {
  ...
  // deserialize() for T (either custom or generated by
  // UPCXX_SERIALIZED_{FIELDS,VALUES}):
  // 1) It may make modifications to the object after constructing it,
  //    but before returning it (valid for deserializing into a void*
  //    since we construct a non-const object, technically UB for
  //    deserializing into an optional<const T>).
  // 2) Its return type usually does not have a const qualifier, so
  //    returning the result of construct() would implicitly drop the
  //    qualifier, which does not compile.
  namespace detail {
    template<typename T>
    struct serialization_storage_wrapper;

    // This specialization is used to deduce the deserialized type for
    // a type T with custom serialization. We return nullptr to be
    // compatible with any return type of deserialize(), as long as it
    // is some pointer type.
    template<>
    ...
    // type, i.e. the actual type of the object constructed in
    // deserialization.
    template<typename T>
  ...
  struct serialization<std::basic_string<CharT, Traits, Alloc>> {
    ...
    static Str* deserialize(Reader &r, Storage storage) {
      ...
      return storage.construct(p, n, std::move(a));
    }
